...
export default class OptionsGamepadScene extends Phaser.Scene {
  ...
  create() {
    ...
    this.add.text(width/2, 20, 'Gamepad Controls', { fontFamily:'monospace', fontSize:'14px', color:'#ffffff' }).setOrigin(0.5,0)
    ...
    const back = this.add.text(width/2, height - 18, 'Back', { fontFamily:'monospace', fontSize:'10px', color:'#ffffff', backgroundColor:'#00000066', padding:{x:4,y:2} }).setOrigin(0.5).setInteractive({ useHandCursor:true })
    ...
    const updateFocus = () => { const w = rows[sel].value; focus.setPosition(w.getCenter().x, w.getCenter().y); focus.setSize(w.width+6, w.height+6) }
